...
# Initialize Pygame
pygame.init()
screen = pygame.display.set_mode((800, 480))  # Windowed resolution
...
class MockOBD:
    ...
    def query(self, cmd):
        if cmd == "GET_DTC":
            ...
            return MockOBDResponse(10 + 5 * math.sin(time.time() * 0.5))  # Mock timing advance
        elif cmd == "ENGINE_LOAD":
            return MockOBDResponse(50 + 25 * math.sin(time.time() * 0.5))  # Mock engine load
        elif cmd == "THROTTLE_POS":
            return MockOBDResponse(30 + 20 * math.sin(time.time() * 0.5))  # Mock throttle position
        elif cmd == "MAF":
            return MockOBDResponse(100 + 50 * math.sin(time.time() * 0.5))  # Mock MAF
        elif cmd == "RPM":
            return MockOBDResponse(4000 + 3000 * math.sin(time.time() * 0.5))
        elif cmd == "SPEED":
            return MockOBDResponse(100 + 60 * math.sin(time.time() * 0.5))
        elif cmd == "COOLANT_TEMP":
            return MockOBDResponse(185 + 35 * math.sin(time.time() * 0.5))
        elif cmd == "INTAKE_TEMP":
            return MockOBDResponse(70 + 50 * math.sin(time.time() * 0.5))
        elif cmd == "FUEL_LEVEL":
            return MockOBDResponse(50 + 50 * math.sin(time.time() * 0.5))
        elif cmd == "ELM_VOLTAGE":
            return MockOBDResponse(13.5 + 2 * math.sin(time.time() * 0.5))
...
def draw_button(surface, x, y, width, height, text, active=False):
    ...
    font = pygame.font.Font("Race Sport.ttf", 18)
...
def draw_digital_box(surface, x, y, width, height, value, max_value, label, units="", decimal_places=1):
    pygame.draw.rect(surface, (20, 20, 40), (x - width // 2, y - height // 2, width, height))
    pygame.draw.rect(surface, BLUE, (x - width // 2, y - height // 2, width, height), 2)
    font_value = pygame.font.Font("Race Sport.ttf", 40)
    font_label = pygame.font.Font("Race Sport.ttf", 20)
    value_text = font_value.render(f"{value:.{decimal_places}f}", True, WHITE)
    label_text = font_label.render(f"{label}", True, WHITE)
    units_text = font_label.render(f"{units}", True, WHITE)
...
def draw_magden_cluster_screen1(surface, x, y):
    ...
    font_logo = pygame.font.Font("Race Sport.ttf", 30)
    logo_text = font_logo.render("magden", True, BLUE)
...
def wrap_text(text, font, max_width):
    ...
    for word in words:
        ...
        if text_surface.get_width() <= max_width:
            ...
            current_line = [word]
            current_width = font.render(word, True, WHITE).get_width()
...
def draw_magden_cluster_screen2(surface, x, y):
    ...
    font_title = pygame.font.Font("Race Sport.ttf", 25)
    title_text = font_title.render("Diagnostic Trouble Codes", True, WHITE)
    surface.blit(title_text, (x - title_text.get_width() // 2, y - 100))

    font_dtc = pygame.font.Font("Race Sport.ttf", 20)
    if dtcs:  # empty lists are False-y
        ...
        for code, desc in dtcs[:5]:
            ...
            wrapped_lines = wrap_text(dtc_text, font_dtc, max_width)
            for line in wrapped_lines:
                text_surface = font_dtc.render(line, True, WHITE)
                surface.blit(text_surface, (x - 350, y + y_offset))
                y_offset += 30
            y_offset += 10  # Extra spacing between DTC entries
    else:
        no_dtc_text = font_dtc.render("No DTCs Found", True, WHITE)
...
def draw_magden_cluster_screen3(surface, x, y):
    ...
    font_title = pygame.font.Font("Race Sport.ttf", 25)
    title_text = font_title.render("Engine Parameters", True, WHITE)
    surface.blit(title_text, (x - title_text.get_width() // 2, y - 100))
